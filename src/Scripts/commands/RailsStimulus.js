...
export default class RailsStimulus {
    updateManifest() {
        if (!isRailsInProject()) {
            console.error(
                'Something went wrong with the update Stimulus manifest command'
            )

            this.showError(
                'Something went wrong with the update Stimulus manifest command. Does the workspace contain a Rails project?'
            )
        ...
        process.onDidExit((status) => {
            if (status === 0) {
                ...
                this.showError(
                    'Something went wrong with the update Stimulus manifest command. Check out the Extension Console for more information.'
                )
            }
        })
