export default class RailsAlternateFile {
    ...
    alternate() {
        ...
        if (
            this.splitPath.indexOf("app") === -1 &&
            this.splitPath.indexOf("test") === -1
        ) {
            this.showError(
                "Couldn't find alternate files. Does the workspace contain a Rails project?"
            );
            return;
        }
        ...
        const rootIndex =
            this.splitPath.indexOf("app") === -1
                ? this.splitPath.indexOf("test")
                : this.splitPath.indexOf("app");
        ...
        if (associatedDir === "test") {
            associatedFileName = currentFileName
                .slice(0, -3)
                .concat("_test.rb");
        } else {
